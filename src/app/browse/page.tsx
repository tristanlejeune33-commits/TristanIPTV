...
export default function BrowsePage() {
  ...
  const groups = useMemo(() => {
    if (!playlist) return [];
    const list = playlist.groupsSorted.map((g) => ({
      name: g,
      count: playlist.groups[g].length,
    }));
    ...
    <div className="mx-auto max-w-[1600px] px-4 md:px-8 py-10">
      <header className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-1">
          {playlist.groupsSorted.length} catégories
        </h1>
        <p className="text-sm text-muted">
          {playlist.channels.length} chaînes au total
        </p>
      </header>
      ...
      {groups.length === 0 ? (
        <p className="text-muted">Aucune catégorie correspondante.</p>
      ) : (
